...
public:
    ...
    void editMesh()
    {
        ...
        LaplaceCotan<double> l;
        atcg::Laplacian<double> laplacian = l.calculate(mesh);
        Eigen::SparseMatrix<double> m_inv = laplacian.M.cwiseInverse();
        // Eigen::SparseMatrix<double> laplace_beltrami = m_inv * laplacian.S;
        Eigen::SparseMatrix<double> euler_lagrange = -ks * laplacian.S + kb * laplacian.S * m_inv * laplacian.S;

        std::vector<Eigen::Triplet<double>> interpVertices;
        std::vector<Eigen::Triplet<double>> fixedVertices;

        for (auto v_it = mesh->vertices_begin(); v_it!= mesh->vertices_end(); v_it++) {
            double dist = mesh->point(*v_it).length();
            int i = v_it->idx();

            if (dist < edit_radius) {  // handle
                fixedVertices.emplace_back(i, i, 1);
                starting_displacement(i, 1) = edit_height;
            }
            else if (dist < region_radius) {
                interpVertices.emplace_back(i, i, 1);
                // starting displacement is zero
            }
            else{
                fixedVertices.emplace_back(i, i, 1);
                // starting displacement is also zero (and should stay zero)
            }
        }

        Eigen::SparseMatrix<double> interpSelector(mesh->n_vertices(), mesh->n_vertices());
        Eigen::SparseMatrix<double> fixedSelector(mesh->n_vertices(), mesh->n_vertices());

        interpSelector.setFromTriplets(interpVertices.begin(), interpVertices.end());
        fixedSelector.setFromTriplets(fixedVertices.begin(), fixedVertices.end());

        Eigen::SparseMatrix<double> op = interpSelector * euler_lagrange + fixedSelector;
